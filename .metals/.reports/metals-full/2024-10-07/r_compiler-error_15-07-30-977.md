...
object Main {
  ...
  def main(args: Array[String]): Unit = {
              ...
              case JsSuccess(apiResponse, _) =>
                apiResponse.data match {
                  case Some(data) =>
                    ...
                    data.lastConnStart.foreach { lastConnStart =>
                      println(s"Fecha de última conexión: $last@@ConnStart")
                    }
                  case None =>
                    println(s"Error en la solicitud para ICC: ${icc.trim}, Mensaje: ${apiResponse.message}")
                }
              case JsError(errors) =>
    ...
    for (resultado <- resultados) {
      println(s"ICC: ${resultado.icc}, Fecha de última conexión: ${resultado.lastConnStart.getOrElse("No disponible")}")
    }
  }
}
```
